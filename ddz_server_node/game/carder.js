...
module.exports = function(){
    ...
    that.splitThreeCards = function(){
		//console.log("发牌开始 "+JSON.stringify(this.card_list));
        var threeCards = {}
        for(var i=0;i<17;i++){
           for(var j=0;j<3;j++){
       ...
       // console.log("分牌 ",JSON.stringify([threeCards[0],threeCards[1],threeCards[2],this.card_list]))
    ...
    that.toCardlist=function(cards_){
		//转换 
		//[{"cardid":3,"card_data":{"index":3,"value":13,"shape":4}},
		//{"cardid":0,"card_data":{"index":0,"value":13,"shape":1}}]
		//成
          //{card_counts:[2],
		  //card_values:[13]
		  //card_valuedetails:[[4,1]]
		  var cards=cards_;
		  var tmp_cards={};
		  ...
		  for(var i=0;i<cards.length;i++){
			...
			var value=0;
			var card_data=card;
			if(typeof(card["card_data"])!="undefined"){
				card_data=card["card_data"];
			}
			if(card_data["value"]!=null)//普通牌值
			{
			  value=card_data["value"]
			}
			if(card_data["king"]!=null){//大小鬼
				value=card_data["king"]
			}
			var value_key=""+value; 
			  if(tmp_cards[value_key]==null){
				  tmp_cards[value_key]={"count":0,"value":value,"valuedetail":[]}
			  }
			  
			  tmp_cards[value_key]["value"]=value;
			  
			  tmp_cards[value_key]["count"]=tmp_cards[value_key]["count"]+1;
			  tmp_cards[value_key]["valuedetail"].push(card_data["shape"])
		  }
		 //console.log("tmp_cards:",JSON.stringify(tmp_cards)); 
		 var t_cards=tmp_cards;
		 tmp_cards=[];
		 for(var i in t_cards)
		 {
			 tmp_cards.push(t_cards[i]);
		 }
		  
		 tmp_cards.sort(function(a,b){
		 ...
		 //console.log("sort ed tmp_cards:",JSON.stringify(tmp_cards));
		// console.log("牌型:",that.toCountArray(tmp_cards).toString());
		 //console.log("牌值:",that.toValueArray(tmp_cards).toString());
		 
		// console.log(that.printCard(tmp_cards));
	     return tmp_cards;	
	}
	
	that.compareCard=function(cards0_,cards1_){//牌型权重 王炸 3 ,炸弹 2,其它 1 ;权重大 可以吃权重小的牌,权重相同,则比较牌型;牌型相同则比较牌值,牌型不同则返回false
		//console.log("比较 ",JSON.stringify(cards0_))
	   var cards0=that.toCardlist(cards0_);
	   //console.log("和 ",JSON.stringify(cards1_))
		var cards1=that.toCardlist(cards1_);
		//console.log("牌值权重 ",that.weightOfCard(cards0)+" "+that.weightOfCard(cards1));
		if(that.weightOfCard(cards0)!=that.weightOfCard(cards1)){
			//console.log("有炸弹 ",that.weightOfCard(cards0)+" "+that.weightOfCard(cards1))
			return that.weightOfCard(cards0)-that.weightOfCard(cards1)
		} 
		 
		 if(that.toCountArray(cards0).toString()!=that.toCountArray(cards1).toString())
	     {
			console.log("无法比较,牌型不同 ",that.printCard(cards0)+" : "+that.printCard(cards1));
			return false;
		 }
		 var values0=that.toValueArray(cards0);
		 var values1=that.toValueArray(cards1);
		 console.log("牌值比较 ",that.printCard(cards0)," "+(values0[0]-values1[0])+" ",that.printCard(cards1));	 
		 return values0[0]-values1[0];
	}
	that.weightOfCard=function(cards_){//牌型权重 王炸 3 ,炸弹 2,其它 1 ;权重大 可以吃权重小的牌,权重相同,则比较牌值
		var cards=cards_;
		//that.toCardlist(cards_);
		//console.log("权值 "+that.toValueArray(cards));
		 if(that.toValueArray(cards).toString()=="15,14"){// 王炸 
	       return 3;
		 }
		 if(that.toCountArray(cards).toString()=="4"){//炸弹
			return 2; 
		 }
		 return 1;// 其它牌型权重
	}
	...
	that.toInfoArray=function(cards){
		var values=[];
		for(var i in cards){
			var card=cards[i];
			values.push(card.getInfo())
		}
		return values
	}
	that.printCard=function(cards){
		var html="";
		for(var i in cards){
			var card=cards[i];
			for(var j=0;j<card.count;j++){
				html+=cardvalues[card.value]+","
			}
		}
		//console.log(html);
		return html;
    ...
    that.compareWithCard = function(last_cards,current_cards){
		...
		//that.toCardlist(last_cards);
		//that.toCardlist(current_cards);
		
		var result=that.compareCard(last_cards,current_cards);
		if(result===false){//牌型不同
			return false;
		}
		if(result<0){// 比上次牌大
			return true;
